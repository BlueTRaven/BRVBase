...
namespace BRVBase
{
	...
	public class TiledLoader
	{
		...
		public TiledMap LoadMap(TmxMap map)
		{
			...
			outMap.backgroundColor = new RgbaFloat(map.BackgroundColor.R, map.BackgroundColor.G, map.BackgroundColor.B, 255);
				...
				if (layer is TmxLayer tileLayer)
				{
					...
					for (int x = 0; x < map.Width; x++)
					{
						for (int y = 0; y < map.Height; y++)
						{
							...
							if (outputLoadMessage)
							{
								LoadMessage = "Blanking tiles...\n(Layer: " + layer.Name + " (" + layerIndex + "/" + (map.Layers.Count - 1) + ") " + iter + "/" + tileLayer.Tiles.Count + ")";
							}
						}
					}
